...
export default function Login() {
    ...
    const { toggleLogin, setToggleLogin, loginVisible, setLoginVisible, credentials, setCredentials } = context;
    ...
    const onHandleCredentialsChange = (e)=>{
        setCredentials({...credentials, [e.target.name]: e.target.value});
    }
    return (
        <div id="loginModal" className={`Login fixed top-[40px] flex justify-center w-full z-40 ${toggleLogin ? '' : 'hidden'}`}>
            <div className="bg-white rounded lg:w-1/3 md:w-1/2 w-full p-10 mt-16 shadow-black shadow-lg">
                {/* login */}
                <div className='flex justify-between'>
                <div className={`${loginVisible ? '': 'hidden'}`}>
                    <p tabIndex="0" className="focus:outline-none text-2xl font-extrabold leading-6 text-gray-800">Login to your account</p>
                    <p tabIndex="0" className="focus:outline-none text-sm mt-4 font-medium leading-none text-gray-500">Dont have account? <button onClick={handleLoginVisible} className="hover:text-gray-500 focus:text-gray-500 focus:outline-none focus:underline hover:underline text-sm font-medium leading-none  text-gray-800 cursor-pointer"> Sign up here</button></p>
                </div>
                {/* signup */}
                <div className={`${loginVisible ? 'hidden': ''}`}>
                    <p tabIndex="0" className="focus:outline-none text-2xl font-extrabold leading-6 text-gray-800">Signup with your account</p>
                    <p tabIndex="0" className="focus:outline-none text-sm mt-4 font-medium leading-none text-gray-500">Already have account? <button onClick={handleLoginVisible} className="hover:text-gray-500 focus:text-gray-500 focus:outline-none focus:underline hover:underline text-sm font-medium leading-none  text-gray-800 cursor-pointer"> Login here</button></p>
                </div>
                <AiOutlineClose className='text-3xl cursor-pointer hover:text-[#2699fb]' onClick={()=>{setToggleLogin(false); setLoginVisible(true); setCredentials({email: "", password: ""});}} />
                </div>
                ...
                <div>
                    ...
                    <input aria-labelledby="email" type="email" name='email' onChange={onHandleCredentialsChange} value={credentials.email} className="bg-gray-200 border rounded  text-lg font-medium leading-none text-gray-800 py-3 w-full pl-3 mt-2" />
                </div>
                <div className="mt-6  w-full">
                    <label htmlFor="pass" className={`text-sm font-medium leading-none text-gray-800 ${loginVisible ? '': 'hidden'}`}>
                        Password
                    </label>
                    <label htmlFor="pass" className={`text-sm font-medium leading-none text-gray-800 ${loginVisible ? 'hidden': ''}`}>
                        New Password
                    ...
                    <div className="relative flex items-center justify-center">
                        <input id="pass" type="password" name='password' onChange={onHandleCredentialsChange} value={credentials.password} className="bg-gray-200 border rounded  text-lg font-medium leading-none text-gray-800 py-3 w-full pl-3 mt-2" />
                ...
                <div className={`mt-8 ${loginVisible ? '': 'hidden'}`}>
                    <button className="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 text-sm font-semibold leading-none text-white focus:outline-none bg-indigo-700 border rounded hover:bg-indigo-600 py-4 w-full">Login to my account</button>
                </div>
                <div className={`mt-8 ${loginVisible ? 'hidden': ''}`}>
